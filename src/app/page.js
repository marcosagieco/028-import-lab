    ...
    return (
      <div className="bg-[#f2f2f2] text-[#111111] font-poppins flex flex-col relative pb-20 md:pb-0 min-h-screen selection:bg-[#fcdb00] selection:text-[#111111]">
        <style dangerouslySetInnerHTML={{__html: `
          ...
          /* --- ANIMACIONES SENIOR PARA LA RULETA --- */
          
          /* 1. Shimmer (Destello de luz que cruza el botón) */
          @keyframes shimmer {
      ...
      {/* --- MODAL RULETA HOT SALE (VIP DARK MODE) --- */}
        {/* --- MODAL RULETA HOT SALE (NIVEL APPLE ENGINEERING - VIP DARK MODE) --- */}
        {showRouletteModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            
            {/* FONDO GENERAL (Más transparente para que se note el efecto del modal) */}
            <div className="absolute inset-0 bg-black/40 backdrop-blur-md transition-opacity duration-500" onClick={() => !isSpinning && setShowRouletteModal(false)}></div>
            
            {/* EL VERDADERO APPLE GLASS (Transparencia, Blur extremo, Bordes de luz reflectante) */}
            <div className="relative w-full max-w-[480px] rounded-[2.5rem] bg-[#111111]/40 backdrop-blur-[40px] border-[0.5px] border-white/20 shadow-[0_20px_60px_rgba(0,0,0,0.6),inset_0_1px_1px_rgba(255,255,255,0.15)] p-8 pt-14 flex flex-col items-center animate-in zoom-in-95 duration-500 overflow-hidden">
              ...
              {/* SPOTLIGHT DE ESTUDIO (Foco de luz suave detrás de la mascota para darle volumen) */}
              <div className="absolute top-[10%] right-[10%] w-[300px] h-[300px] bg-white/10 blur-[100px] rounded-full pointer-events-none z-0"></div>

              {/* TU IMAGEN DE MASCOTA INTACTA */}
              <img 
              ...
              <div className="relative z-30 w-full h-[95px] flex items-center justify-center mb-6 mt-2 pointer-events-none">
                <img 
                  ...
                  alt="Hot Sale 028" 
                  className="absolute top-1/2 left-1/9 -translate-x-1/3 -translate-y-[50%] w-[430px] md:w-[450px] max-w-none drop-shadow-[0_15px_25px_rgba(0,0,0,0.7)]" 
                />
              </div>
              
              {/* --- ARTEFACTO DE LA RULETA (Premium Metal & Glass) --- */}
              <div className="relative w-80 h-80 md:w-84 md:h-84 mb-10 mt-2 z-20 flex items-center justify-center">

                {/* HALO GLOW AMARILLO SUAVE DETRÁS DE LA RULETA (Para separarla físicamente del fondo) */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[115%] h-[115%] bg-[radial-gradient(circle,rgba(252,219,0,0.1)_0%,transparent_65%)] blur-[25px] pointer-events-none z-0"></div>
                
                {/* ANILLO ESTRUCTURAL DE METAL PESADO (Simula Gunmetal Anodizado) */}
                <div className="absolute inset-0 rounded-full bg-gradient-to-b from-[#3a3a3a] via-[#111111] to-[#050505] p-[8px] shadow-[0_25px_60px_rgba(0,0,0,0.9),inset_0_3px_5px_rgba(255,255,255,0.2),inset_0_-3px_5px_rgba(0,0,0,0.5)] z-10">
                  
                  {/* LA RULETA EN SÍ (Efecto tazones de cerámica/material) */}
                  <div 
                    className="w-full h-full rounded-full relative overflow-hidden shadow-[inset_0_0_30px_rgba(0,0,0,0.6),inset_0_10px_20px_rgba(0,0,0,0.8)] border-4 border-[#050505]"
                    style={{ 
                      ...
                      return (
                        <div key={idx} className="absolute inset-0 z-0" style={{ transform: `rotate(${angle + 18}deg)` }}>
                          <div className="absolute top-0 left-0 right-0 h-1/2 flex items-start justify-center pt-5 md:pt-6">
                            <span 
                              className={`font-bebas uppercase whitespace-nowrap drop-shadow-[0_2px_3px_rgba(0,0,0,0.5)] ${prize.text.length > 12 ? 'text-[9px] md:text-[11px] tracking-normal' : 'text-[12px] md:text-[14px] tracking-wider'}`} 
                ...
                {/* CAPA DE CRISTAL / ACRÍLICO (Reflejo de luz estático por encima de la ruleta que gira) */}
                <div className="absolute inset-3 rounded-full pointer-events-none z-20">
                    {/* Mancha de luz superior (Brillo de acrílico) */}
                    <div className="absolute top-[1%] left-[15%] right-[15%] h-[40%] bg-gradient-to-b from-white/15 via-white/5 to-transparent rounded-full blur-[3px]"></div>
                    {/* Sombra inferior (Profundidad cóncava) */}
                    <div className="absolute bottom-0 left-0 right-0 h-[30%] bg-gradient-to-t from-black/40 to-transparent rounded-b-full blur-[5px]"></div>
                </div>

                {/* PUNTERO MECÁNICO ADICIONAL (Pequeño triángulo metálico arriba) */}
                <div className="absolute top-[-2px] left-1/2 -translate-x-1/2 w-8 h-10 z-40 flex flex-col items-center pointer-events-none drop-shadow-[0_5px_5px_rgba(0,0,0,0.5)]">
                    <div className="w-full h-8 bg-gradient-to-b from-[#666] to-[#222] rounded-t-sm" style={{ clipPath: 'polygon(50% 100%, 0 0, 100% 0)' }}></div>
                    <div className="w-2 h-2 bg-[#111] rounded-full mt-[-6px] border border-[#444]"></div>
                </div>

                {/* EJE CENTRAL (Botón mecánico mate) */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 md:w-14 md:h-14 bg-gradient-to-br from-[#555] via-[#1a1a1a] to-[#0a0a0a] rounded-full border-2 border-[#222] shadow-[0_10px_20px_rgba(0,0,0,0.9),inset_0_3px_5px_rgba(255,255,255,0.15)] z-30 pointer-events-none"></div>

                {/* PUNTERO INTACTO DEL USUARIO (Mascota) */}
                <img 
              ...
              {/* BOTÓN CTA PREMIUM (Física 3D + NUEVA Animación SHIMMER Hipnótica) */}
              <div className="relative w-full z-30 group active:scale-[0.98] transition-transform">
                  <button onClick={handleSpinRoulette} disabled={isSpinning} className={`w-full py-4 md:py-5 rounded-2xl font-bebas text-3xl md:text-4xl uppercase tracking-wider flex items-center justify-center gap-3 relative overflow-hidden transition-all duration-300
                    ${isSpinning 
                      ...
                      {/* CAPA DE SHIMMER (Luz cruzando de izquierda a derecha) */}
                      {!isSpinning && (
                          <div className="absolute inset-0 w-[200%] h-full animate-shimmer opacity-40 bg-gradient-to-r from-transparent via-white to-transparent" style={{ transform: 'skewX(-20deg)' }}></div>
                      )}
                      
                      {/* CONTENIDO DEL BOTÓN INTACTO */}
                      {isSpinning ? <><i className="fas fa-circle-notch fa-spin text-2xl"></i> Girando...</> : '¡PROBA SUERTE!'}
                  </button>
              </div>

              {/* TEXTO DE URGENCIA INTACTO */}
